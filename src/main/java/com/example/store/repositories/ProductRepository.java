...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    // @Query(@Param("min") BigDecimal min, @Param("max") BigDecimal max, value = "select * from products p where p.price between :min and :max order by p.name", nativeQuery = true)
    // List<Product> findProducts(BigDecimal min, BigDecimal max);

    // @Query("select p from Product p join p.category c where p.price between :min and :max order by p.name")
